...
N_VIEWS     = 20  #tot dataset-ul

MIN_TRACK_LENGTH   = 2       #acceptam si perechi (2 vederi) - mai multe puncte dar mai putin fiabile
MAX_REPROJ_ERROR   = 20.0    #prag pe eroarea medie (inainte de BA) - relaxat pt. densitate
RANSAC_THRESHOLD   = 4.0     #relaxat agresiv pt. mai multi inlieri
LOWE_RATIO         = 0.85    #relaxat agresiv (default era 0.75) pt. mult mai multe match-uri

#SIFT mai agresiv - extrage features si din zone cu contrast redus
SIFT_CONTRAST_THR  = 0.02    #default OpenCV = 0.04; 0.02 = mult mai dens
SIFT_EDGE_THR      = 15      #default = 10; mai permisiv

#Bundle Adjustment - optimizare globala post-triangulare
USE_BUNDLE_ADJUSTMENT   = True
BA_MAX_ITERATIONS       = 50
POST_BA_REPROJ_FILTER   = 3.0   #dupa BA putem fi mult mai stricti

#ajustare: daca vrei DOAR puncte foarte fiabile (din >=3 vederi), seteaza MIN_TRACK_LENGTH=3
...
# ------------------- 4. SELECTIE: FOLOSIM TOATE IMAGINILE -------------------
#ocolim ImageSelector si folosim tot dataset-ul in ordinea din director
...
#5a. construim tracks peste toate vederile
print("\n[1/2] Construire tracks (union-find peste match-uri RANSAC)...")
...
#5b. triangulare DLT per track + filtrare consistenta
print(f"\n[2/2] Triangulare DLT multi-view + filtru eroare max < {MAX_REPROJ_ERROR}px...")
...
# ------------------- 6. POST-FILTRARE IQR (outlieri 3D extremi) -------------------
if len(points_3d) > 10:
...
if len(points_3d) > 0:
    ...
    #varianta 1: TOATE punctele 3D proiectate pe aceasta vedere
    #(unele pot sa nu fi fost triangulate din aceasta imagine)
    ...
    #varianta 2: DOAR punctele care au fost observate in aceasta vedere
    #(adica track-ul lor contine view_idx) - verificare stricta
